...
const createServer = () => {
  app.get('/hello', (request, response) => response.send('Hey everyone'))
  app.get('/aboutMe', (request, response) => response.send('My name is Byron Keyes'))
  app.get('/favoriteFoods', (request, response) => response.send(['burgers', 'tacos']))
...
const count = (target, input) => {
  const result = input.reduc((acc, cur) => {
...
const totalSum = (input) => {
  let numberArray = input.reduce((acc, val) => acc.concat(val).map(number => number))
    .reduce(function(a,b){
      return a + b;
    }, 0);
  return numberArray
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(stringArray => stringArray.filter(value => typeof(value) !== 'string'))
    .map(stringlessArray => stringlessArray.filter((num => num % 5 === 0)))
    .map(modFilteredArray => modFilteredArray.map(number => Math.pow(2, number)));
};
/* ------------------------------------------------------------------------------------------------
